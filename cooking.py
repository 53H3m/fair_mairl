...
from demos import Demonstrations, CompiledDemonstrations
from irl_types import IRLParams, Config, RunData, MHConfig
from games.regularised_mdp_solver import qre_gap
from games.cooking_mg import LightCollaborativeCooking, State, PlayerState, hashabledict
...
from tf_models import SoftmaxPolicies, StateSwapReward, convert_to_irl_params
...
def diverse_samples(n, a, b, num_candidates=3):
    samples = [np.random.uniform(a, b)]
    for _ in range(n - 1):
        candidates = np.random.uniform(a, b, size=num_candidates)
        dists = np.array([
            min(abs(c - s) for s in samples)
            for c in candidates
        ])
        # Choose the candidate with max min-distance
        best_candidate = candidates[np.argmax(dists)]
        samples.append(best_candidate)
    return tuple(np.array(samples).tolist())


def generate_solution(true_game, config):
    fairness = (0., 1., 0.5)

    truth = IRLParams(
        fairness_levels=fairness[:config.pool_size],
        reward_func=true_game.reward_func,
    )
...
def run_for_config(config: Config):
    ...
    cooking_mg = LightCollaborativeCooking(**partial_mg_config)
    truth = generate_solution(cooking_mg, config)

    demos = CompiledDemonstrations(
        partial_mg_config,
        gamma,
        lambda_reg,
        true_mg=cooking_mg
...
def run_ula(
    ...
    reward_model = StateSwapReward(
        num_agents=config.pool_size,
        num_states=demos.base_mdp.num_states,
        num_actions=demos.base_mdp.num_actions,
        state_swaps=demos.base_mdp.state_swaps,
        fairness_bounds=(0., 1.)
    )
...
def run_ula_porp(
    ...
    policies = list(np.array(sampled_policies)[np.random.choice(len(sampled_policies), 200)])
    ...
    return policies, sampled_rewards, {"policy_posterior_time": t2 - t, "reward_posterior_time": t3 - t2, "porp_time": t3 - t}
...
def evaluate_rewards(configs: List[Config]):
    ...
    for config in configs:
        ...
        fairness_error, base_reward_error, false_positives, true_positives = compute_reward_error(run_data)
        evaluation[path]["fairness_error"].append(fairness_error)
        evaluation[path]["base_reward_error"].append(base_reward_error)
        evaluation[path]["false_positives"].append(false_positives)
        evaluation[path]["true_positives"].append(true_positives)
...
def compute_reward_error(run_data: RunData):
    ...
    fairness_errors = tree.map_structure(
        lambda a, b: np.float16(np.abs(a - b)),
        sampled_fairness,
        true_fairness
    )
    ...
    def _false_positives(p, t, threshold=0.5):
        return np.int8(np.logical_and(p > threshold, ~(t > threshold)))

    def _true_positives(p, t, threshold=0.5):
        return np.int8(np.logical_and(p > threshold, t > threshold))

    false_positives = tree.map_structure(
        lambda a, b: _false_positives(a, b),
        sampled_base_rewards,
        tile_base_rewards
    )

    true_positives = tree.map_structure(
        lambda a, b: _true_positives(a, b),
        sampled_base_rewards,
        tile_base_rewards
    )

    return fairness_errors, base_reward_errors, false_positives, true_positives
